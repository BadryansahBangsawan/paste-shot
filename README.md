...
**Copy each macOS screenshot to the clipboard. ⌘V pastes the PNG — or the path in Terminal.**
...
| **Clipboard** | PNG + TIFF. Quoted `last.png` path as `text/plain` only while Terminal is frontmost. A browser that prefers text still gets the image. |
...
3. ⌘V pastes the **image** (Notes, Slack, browsers). Click Terminal, then ⌘V pastes the **quoted path** to `last.png`.
...
| Website ⌘V pastes `'…/PasteShot/last.png'` | That page read `text/plain`. Click the browser so it is frontmost (path is stripped), or `defaults delete engineer.badry.pasteshot copyPath`. |
| Terminal ⌘V is empty | Click the Terminal window, then ⌘V. Need **1.0.11+**. Force the path always: `defaults write engineer.badry.pasteshot copyPath -bool true`. |
...
The quoted path is `text/plain` only while Terminal is frontmost. Click the browser, then ⌘V. `copyPath` forces the string in every app.

**How do I paste the path in Terminal?**  
Click Terminal, then ⌘V. The file is `~/Library/Application Support/PasteShot/last.png`. To always put the quoted path on the clipboard: `defaults write engineer.badry.pasteshot copyPath -bool true`.
