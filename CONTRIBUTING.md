...
Thanks for improving Paste Shot.

End users install with `brew tap BadryansahBangsawan/mac-menu-apps` and `brew install --cask paste-shot`. This file is for source contributors.
...
Do not commit `dist/`, `.build/`, `.swiftpm/`, or secrets. Do not run `dist/` next to `/Applications/PasteShot.app` (same bundle ID).
